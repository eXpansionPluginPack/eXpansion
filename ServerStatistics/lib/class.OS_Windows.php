...
class OS_Windows {
    ...
    public function getLoad() {
        ...
        /* testing query for perfmon data.. works but can't get load-average
          ...
          $value = $refresher->AddEnum($wmi, "Win32_PerfFormattedData_PerfOS_Processor");

          $refresher->Refresh();
          // ProcessorQueueLength
          foreach ($value->ObjectSet as $key => $set) {
          var_dump($set->PercentProcessorTime);
          // var_dump($set->Timestamp_Sys100NS);
          }
         */
        //$test = $this->wmi->ExecQuery("SELECT * FROM Win32_PerfFormattedData_PerfOS_System");
