...
// only special maps under 8 seconds are allowed
	...
	try {
		...
		\ManiaLive\Utilities\Console::println("[Dedimania] Fault from dedimania server: " . $array[0]['faultString']);
		return;
	    }

	    if (!empty($array[0][0]['Error'])) {
		// $this->connection->chatSendServerMessage("Dedimania error: " . $array[0][0]['Error']);
		\ManiaLive\Utilities\Console::println("[Dedimania] Error from dedimania server: " . $array[0][0]['Error']);
		return;
	    }
...
// best b better than best a
	...
	if (!empty($data[0][0]['Error'])) {
	    Console::println("[Dedimania] Authentication Error occurred: " . $data[0][0]['Error']);
	    return;
	}
    }
