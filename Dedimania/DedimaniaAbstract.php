...
abstract class DedimaniaAbstract extends \ManiaLivePlugins\eXpansion\Core\types\ExpPlugin implements \ManiaLivePlugins\eXpansion\Dedimania\Events\Listener
{
	...
	public function onDedimaniaNewRecord($record)
	{
		try {
			...
			$this->exp_chatSendServerMessage(
					$this->msg_newRecord, $recepient, array(\ManiaLib\Utils\Formatting::stripCodes($record->nickname, "wos"), $record->place, $time)
			);
		} catch (\Exception $e) {
	...
	public function onDedimaniaRecord($record, $oldRecord)
	{
		...
		try {
			...
			$this->exp_chatSendServerMessage(
					$this->msg_record, $recepient, array(\ManiaLib\Utils\Formatting::stripCodes(
						$record->nickname, "wos"
				), $record->place, $time, $oldRecord->place, $diff)
			);
	...
	public function showCps($login)
	{
		...
		try {
			...
			$window->setTitle(__('Dedimania Cps on a Map', $login));
			$window->centerOnScreen();
			$window->populateList($this->records);
