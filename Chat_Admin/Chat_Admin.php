...
class Chat_Admin extends \ManiaLivePlugins\eXpansion\Core\types\ExpPlugin {
	...
	function blacklist($fromLogin, $params) {
		...
		$player = Storage::GetInstance()->getPlayerObject($params[0]);
		if ($player == null) {
			$this->sendErrorChat($fromLogin, 'Player %variable%' . $param[0] . '$0ae doesn\' exist.');
			return;
		}
		...
		try {
			$this->connection->banAndBlackList($player, $msg, true);
			$this->exp_chatSendServerMessage('%admina_action%Admin %variable%' . $admin->nickName . '$z$s%admina_action% blacklists the player %variable%' . $nickname);
		} catch (\Exception $e) {
	...
	function forceSpec($fromLogin, $params) {
		...
		$plNick = $player->nickName;
		$this->mlepp->sendChat('%admin_action%Admin %variable%' . $admin->nickName . '$z$s%admina_ction% Forces the player %variable%' . $player->nickName . '$z$s%admin_action% to Spectator.');
	}
	...
	function skipTrack($fromLogin, $params) {
		try {
			$this->connection->nextMap(Storage::GetInstance()->gameInfos->gameMode == \DedicatedApi\Structures\GameInfos::GAMEMODE_CUP);
			$admin = Storage::GetInstance()->getPlayerObject($fromLogin);
	...
	function restartTrack($fromLogin, $params) {
		try {
			$this->connection->restartMap(Storage::GetInstance()->gameInfos->gameMode == \DedicatedApi\Structures\GameInfos::GAMEMODE_CUP);
			$admin = Storage::GetInstance()->getPlayerObject($fromLogin);
