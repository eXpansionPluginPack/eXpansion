...
class AutoUpdate extends \ManiaLivePlugins\eXpansion\Core\types\ExpPlugin
{
	...
	exec("cd ./vendor/maniaplanet/dedicated-server-api && git checkout $branch && git pull --rebase", $results, $ret);
	if ($ret != 0) {
	...
	exec("cd ./libraries/ManiaLivePlugins/eXpansion && git checkout $branch && git pull --rebase", $results, $ret);
	if ($ret != 0) {
