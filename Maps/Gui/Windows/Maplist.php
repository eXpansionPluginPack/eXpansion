...
// attempt to find the substring of $haystack most closely matching $needle
	...
	usort($array, function($a, $b) {
		...
		return 0;
	    else if (!isset($a->mapRating))
		return -1;
	    else if (!isset($b->mapRating))
		return 1;
	    else
	...
	usort($array, function($a, $b) use ($login) {
		...
		return 0;
	    else if (!isset($a->localRecords))
		return -1;
	    else if (!isset($b->localRecords))
		return 1;
	    else if (!isset($a->localRecords[$login]) && !isset($b->localRecords[$login]))
		return 0;
	    else if (!isset($a->localRecords[$login]))
		return -1;
	    else if (!isset($b->localRecords[$login]))
		return 1;
	    else
