...
function autotrackmanager()
		...
		Console::println('['.date('H:i:s').'] [eXpansion] [ATM] Karma: '.$rating.'');
		Console::println('ATM Debug: RatingTotal: '.$rating.', Players Voted Total: '.$this->ratingTotal.', ratio: '.( ( $votecount>0 )?( round($rating/$votecount,2) ):('n/a') ));
		if($votecount >= $this->config->MINVotes && $rating/$votecount <= $this->config->integervalue) 
			{
			Console::println('['.date('H:i:s').'] [eXpansion] [ATM] Karma: removal test');
			Console::println('ATM Debug: Track too bad: '.$this->storage->currentMap->name);
			 /**
...
Console::println("Error:\n".$e->getMessage());
		...
		Console::println('['.date('H:i:s').'] [eXpansion] [ATM] Removed current track from the tracklist.');
