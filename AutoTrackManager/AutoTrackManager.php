...
class AutoTrackManager extends \ManiaLivePlugins\eXpansion\Core\types\ExpPlugin {
    ...
    function onPlayerConnect($login, $isSpectator) {
        ...
        $msg = 'AutoTrackManager version 1.0a loaded successfully! Type /atm';
    ...
    function autotrackmanager() {
        ...
        if ($votecount >= $this->config->MINVotes && $rating / $votecount <= $this->config->integervalue) {
            $this->console('[' . date('H:i:s') . '] [eXpansion] [ATM] Karma: removal of Map');
            ...
            $query = $this->db->execute($q);
            $q = "DELETE FROM exp_rating  WHERE uid = " . $this->db->quote($this->storage->currentMap->uId) . ";";
            $query = $this->db->execute($q);
			$q = "DELETE FROM exp_records WHERE record_challengeuid = " . $this->db->quote($this->storage->currentMap->uId) . ";";
            $query = $this->db->execute($q);
			$this->console('[' . date('H:i:s') . '] [eXpansion] [ATM] Finished removing all data from database from ' . $this->storage->currentMap->name . '');
        }
    }
