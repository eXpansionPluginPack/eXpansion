...
class AutoLoad extends \ManiaLivePlugins\eXpansion\Core\types\ExpPlugin
{
	...
	if (!empty($recheck)) {
	    ...
	    $this->exp_chatSendServerMessage("couldn't Autoload all required plugins, see console log for more details.");
	...
	if ($this->isInStartList($pluginId) || $pHandler->isLoaded($pluginId)) {
		...
		$this->exp_chatSendServerMessage("#admin_error#This plugin is a core element of eXpansion. It cant be unloaded", $login);
	    } else {
